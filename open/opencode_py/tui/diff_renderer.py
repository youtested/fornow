...
try:
    ...
    Comment = Keyword = Operator = Punctuation = String = Number = None
    Name = Generic = Whitespace = None
    PymError = None

_HUNK_RE = re.compile(r"^@@ -(\d+)(?:,\d+)? \+(\d+)(?:,\d+)? @@")

# pygments base token -> opencode theme scope name
_TYPE_TO_SCOPE = [
    ((Comment,), "comment"),
    ((Keyword,), "keyword"),
...
class _DiffHighlighter:
    ...
    def _get_style(self) -> object:
        ...
        try:
            ...
            styles: dict[str, str] = {}
            for toks, scope in _TYPE_TO_SCOPE:
                for tok in toks:
